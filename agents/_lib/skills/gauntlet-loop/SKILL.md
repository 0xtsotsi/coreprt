...
2. **Set the bar.** If the user supplied a reference (e.g. `bar:comaxx-launch-case`), use it. If not, offer **2 or 3 candidate bars**, one line each, and stop. Wait for their pick. Do not write the prompt yet.
...
## Wiring in CorePrt (2026-08-06)

In the CorePrt fleet, the gauntlet-loop is wired as the **default execution model** for any turn that mentions a `bar:<name>` tag or the `/gauntlet <bar-name>` slash command. The router (fizz) classifies the turn:

- If a `bar:<name>` reference is present, the request becomes a kind:43001 `JOB_REQUEST` with `["gauntlet", "<bar-name>"]` in the tags.
- The assigned builder agent (goji or bumble) runs the work, publishes the result as kind:9.
- A separate Scout critic agent (initially ggcoder-rpc over ken autopilot, replaced by an MCP-style Scout agent in PR-9) is then spawned **with fresh context** to do a blind A/B against the bar.
- The verdict flows back as a kind:1111 (NIP-22) comment on the builder's event: `WIN | LOSE | EQUAL` plus the single biggest remaining gap.
- On `WIN` the loop exits; on `LOSE` or `EQUAL` the loop injects the gap as a follow-up kind:9 to the builder, capped at 5 rounds (configurable).

The bar registry lives at `agents/_lib/skills/gauntlet-loop/bars/<name>.json`. Each entry is a typed `{kind, ref, fetch, comparable}` envelope. See `bars/thecardyard-home.json` for the webrnds-default bar template.
...
Adapted from `gauntlet-loop` by Matt Shumer (robonuggets/gauntlet-loop),
CC BY 4.0. See `LICENSE` and `NOTICE.md` in this directory for attribution.